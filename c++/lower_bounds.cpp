...
void findUpperLowerEnvelope( traj, w, &uppEnv, &lowEnv)
{
    int lenT = traj.rows();
    ...
    for( i=0; i<w; i++)
    {
        for( j=0; j<lenQ_cols; j++)
        {
            uppEnv(i,j) = uppEnv(w,j);
            lowEnv(i,j) = lowEnv(w,j);

            uppEnv(lenQ-w+i,j) = uppEnv(lenQ-w-1,j);        
            lowEnv(lenQ-w+i,j) = lowEnv(lenQ-w-1,j);
        }
    }
...
float Keogh_bounDemLow_SakoeChiba( MatrixXf trajC, MatrixXf trajQ, int w, MatrixXf &upperQ, MatrixXf &lowerQ)
{
    ...
    MatrixXf uppEnv = MatrixXf::Zero( traj.rows(), traj.cols()); 
    MatrixXf lowEnv = MatrixXf::Zero( traj.rows(), traj.cols()); 
...
float Improved_bounDemLow( MatrixXf trajC, MatrixXf trajQ, int w, MatrixXf &uppQ, MatrixXf &lowQ)
{
    ...
    MatrixXf uppEnvQ = MatrixXf::Zero( traj.rows(), traj.cols()); 
    MatrixXf lowEnvQ = MatrixXf::Zero( traj.rows(), traj.cols()); 
...
void Enhanced1_bounDemLow( MatrixXf trajC, MatrixXf trajQ, int w, int v, float d)
{
